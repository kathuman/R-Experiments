...
Key(.8, .68, other=list(cex=.7, title=expression(~~~~~~~~~~~sigma)))
...
# First model stratifies age into tertiles to get more empirical
# estimates of age x cholesterol interaction
...
latex(anova(f), caption='Crudely categorizing age into tertiles',
      file='', size='smaller', label='tab:anova-tertiles')
yl <- c(-1,5)
...
wireframe(qlogis(p) ~ cholesterol*age,
          ...
          zlab=list(label='logit', rot=90), zlim=zl,
...
f <- lrm(sigdz ~ lsp(age,c(46,52,59)) *
         ...
         data=acath)
ltx(f)
latex(anova(f), caption='Linear spline surface',
      file='', size='smaller', label='tab:anova-lsp')   # Table (*\ref{tab:anova-lsp}*)
perim <- with(acath, perimeter(cholesterol, age, xinc=20, n=5))
...
ltx(f)
latex(anova(f), caption='Cubic spline surface',
      file='', size='smaller', label='tab:anova-rcs')   # Table (*\ref{tab:anova-rcs}*)
...
latex(anova(f), caption='Singly nonlinear cubic spline surface',
      file='', size='smaller', label='tab:anova-ria')   # Table (*\ref{tab:anova-ria}*)
...
latex(anova(f), caption='Linear interaction surface',
      file='', size='smaller', label='tab:anova-lia')   # Table (*\ref{tab:anova-lia}*)
...
# Make estimates of cholesterol effects for mean age in tertiles 
# corresponding to initial analysis
mean.age <-
  with(acath,
       as.vector(tapply(age, age.tertile, mean, na.rm=TRUE)))
plot(Predict(f, cholesterol, age=round(mean.age,2), sex="male"),
...
which.influence(f, .4)   # Table (*\ref{table:dfbetas}*)
...
plot(0, 0, ylim=c(0,1), xlim=c(0,1),
     ...
     ylab="Actual Probability", type="n")
...
latex(s, file='', size='Ssize', label='tab:lrm-cholxage-confbar')
